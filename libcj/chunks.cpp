...
reloc_t chunk_emit(runasm::runasm_t &x86, jit_op op) {
  ...
  switch (op) {
  case ins_const:
    x86.MOV(EAX, 0xaabbccddu);
    x86.PUSH(EAX);
    return reloc_t{ x86.head() - 5, reloc_t::RELOC_ABS };

  case ins_drop:
    x86.ADD(ESP, 0xaabbccddu);
    return reloc_t{ x86.head() - 4, reloc_t::RELOC_ABS };

  case ins_frame:
    x86.PUSH(EBP);
    x86.MOV (EBP, ESP);
    x86.SUB (ESP, 0xaabbccddu);
    return reloc_t{ x86.head() - 4, reloc_t::RELOC_ABS };
  ...
  case ins_call:
    x86.CALL();
    x86.PUSH(EAX);
    return reloc_t{ x86.head() - 5, reloc_t::RELOC_REL };

  case ins_ret:
    x86.POP(EAX);
    x86.ADD(ESP, 0xaabbccdd);
    x86.POP(EBP);
    x86.RET();
    return reloc_t{ x86.head() - 6, reloc_t::RELOC_ABS };

  case ins_jmp:
    x86.JMP32(label_t{});
    return reloc_t{ x86.head() - 4, reloc_t::RELOC_REL };
  ...
  return chunk_emit_table(x86.pointer(), op);
}
